...
--                menu:addSlice("Ignition", getTexture("media/ui/vehicles/vehicle_ignitionON.png"), ISBoatMenu.onStartEngine, playerObj)
                ...
                if manualStarter then 
                    if manualStarter:getInventoryItem() and manualStarter:getCondition() > 0 then
                        menu:addSlice(getText("ContextMenu_VehicleStartEngineManual"), getTexture("media/ui/boats/RadialMenu_ManualStarter.png"), ISBoatMenu.onStartEngineManualy, playerObj, boat, manualStarter)
                    else
...
--                    menu:addSlice("You need keys or\nelectricity level 1 and\nmechanic level 2\nto hotwire", getTexture("media/ui/vehicles/vehicle_ignitionOFF.png"), nil, playerObj)
    ...
    if not inCabin and boat:getPartById("Sails") and boat:getPartById("Sails"):getInventoryItem() then
        if boat:getPartById("Sails"):getModData().sailCode ~= 0 then
        ...
        if boat:getPartById("Sails"):getModData().sailCode ~= 1 then
        ...
        if boat:getPartById("Sails"):getModData().sailCode ~= 2 then
    ...
    if cabinLock then
        if not cabinLock:getModData()["AquaCabin_isUnlocked"] then
            if playerObj:getInventory():haveThisKeyId(boat:getKeyId()) or inCabin then
                local func = function() 
                    sendClientCommand(playerObj, 'aquatsar', 'unlockCabin', {boat = boat:getId(), })
                    playerObj:getEmitter():playSound("UnlockDoor")
                end
                menu:addSlice(getText("ContextMenu_Open_Cabin"), getTexture("media/ui/boats/RadialMenu_Door.png"), func)
            else
                ...
                if opener then
                    ...
                    if not tired and not bored and not unhappy then                
                        ...
                        menu:addSlice(getText("ContextMenu_Open_Cabin_Force", string.lower(opener:getName())), getTexture("media/ui/boats/RadialMenu_Door.png"), func)
                    elseif tired then
        ...
        elseif cabinLock:getModData()["AquaCabin_isUnlocked"] and not cabinLock:getModData()["AquaCabin_isLockRuined"] then
            if playerObj:getInventory():haveThisKeyId(boat:getKeyId()) or inCabin  then
                local func = function()
                    sendClientCommand(playerObj, 'aquatsar', 'lockCabin', {boat = boat:getId(), })
                    playerObj:getEmitter():playSound("LockDoor")
                end
...
function ISBoatMenu.onStartEngineManualy(playerObj, boat, manualStarter)
--    local boat = playerObj:getVehicle()
--    if not boat then return end
--    if not boat:isEngineWorking() then return end
--    if not boat:isDriver(playerObj) then return end
    manualStarter:setCondition(manualStarter:getCondition() - 1) -- FIX ME
    boat:transmitPartCondition(manualStarter)
