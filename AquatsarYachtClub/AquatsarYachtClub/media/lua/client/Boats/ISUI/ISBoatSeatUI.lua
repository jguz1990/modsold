...
function ISBoatSeatUI:render()
    ...
    for seat=1,self.vehicle:getMaxPassengers() do
        ...
        local playerHaveAccesToCabin = false
        if self.vehicle:getPartById("CabinLock") and self.vehicle:getPartById("CabinLock"):getModData()["AquaCabin_isUnlocked"] then
            playerHaveAccesToCabin = true
        end
        local isCabin = false
