...
local function initUI()
    if ISEquippedItem.instance and not ISEquippedItem.instance.handMainExtensionUI then
        handMainExtensionUI = UIHandMainExtension:new(44, 11)
        handMainExtensionUI:initialise()
        handMainExtensionUI:setVisible(false)
        handMainExtensionUI.parent = ISEquippedItem.instance
        ISEquippedItem.instance:addChild(handMainExtensionUI)
        ISEquippedItem.instance.handMainExtensionUI = handMainExtensionUI
    end
end
...
local function getInventoryAmmoCount(player, item)
    ...
    local tooltip = getText("Tooltip_TheStar_AmmoCount_Bullets", bulletsCount)
    ...
    if boxType then
        ...
        if boxCount > 0 then
            ...
            if boxBulletsCount > 0 then
                tooltip = tooltip .. getText("Tooltip_TheStar_AmmoCount_Boxes", boxCount * boxBulletsCount)
    ...
    if magazineType then
        ...
        if bulletsInMagazines > 0 then
            tooltip = tooltip .. getText("Tooltip_TheStar_AmmoCount_Magazines", bulletsInMagazines)
...
function TheStar.HandMainExtension.Ammo.onEquipPrimary(player, item)
    ...
    if not item or not TheStar.Utils.isLightSource(item) or not player:isLocalPlayer() then
        hideUI()
    end
...
function TheStar.HandMainExtension.Battery.onEquipPrimary(player, item)
    ...
    if not item or not TheStar.Utils.isRangedWeapon(item) or not player:isLocalPlayer() then
        hideUI()
    end
